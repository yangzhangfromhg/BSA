...
double BSA::get_evolve_state(std::vector<Bird> * T, int * gbest)
{
	double temp = 0.0;
	double re_temp = 0.0;
	...
	{	
		for (int c = 0; c < sut->parameter; c++) 
		{
			temp += std::pow(gbest[c] - (*T)[i].position[c], 2);
		}
		temp = std::sqrt(temp);
		re_temp += temp;
		temp = 0.0;
	}
	re_temp = re_temp / T->size();
	double normal = 0.0;
	for (int c = 0; c < sut->parameter; c++) 
	{
		normal += std::pow(sut->value[c], 2);
	}
	normal = std::sqrt(normal) * 0.5;

	re_temp = re_temp / normal;
	return re_temp;
}
...
int* BSA::Evolve() 
{
	...
	int bestfit = 0;
	//ofstream out;
	//out.open("swarm_state.csv", ios::app);
	//out << "\n\n\n\n\n";
	//initial
	...
	while (true) 
	{
		...
		*/
		if (!SWITCHP) 
		{
			...
			{
				(*i).fitness_now = sut->FitnessValue((*i).position, 0);
			}
		}
		else {
			sut->FitnessValueByParallel(&T, config.population, sut->parameter);
		}
		...
		//计算种群状态
		//double state = get_evolve_state(&T, gBest);
		//std::cout << "swarm state : " << state << std::endl;
		//out << state << std::endl;
		// 终止条件
		...
		// 动态飞行行为
		config.FQ = 5 + int(20 * (bestfit / sut->testcaseCoverMax));
		//小鸟行为
		if (it % config.FQ != 0) 
		{		
			for (std::vector<Bird>::iterator i = T.begin(); i != T.end(); ++i)
			{
				...
				double pro = ((double)(rand() % 1000) / 1000.0) * 0.2 + 0.5;
				if (rand_bsa < pro)
				{
